...
namespace LanguageWorkbench.Algorithms.LanguageWriters
{
	public class CWriter : WriterBase
	{
		...
		public CWriter(String grammarName, String grammarInput, ZipPackage zipPackage)
		{
		...
		public override void Write()
		{
			...
			_zipPackage.AddStream(memoryStream, "main.c");

			bytes = Encoding.UTF8.GetBytes(src.ToString());
			 memoryStream = new MemoryStream();
			 memoryStream.Write(bytes, 0, bytes.Length);
			 memoryStream.Seek(0, SeekOrigin.Begin);
			_zipPackage.AddStream(memoryStream, String.Format("{0}.c", _grammarName));

			bytes = Encoding.UTF8.GetBytes(header.ToString());
			memoryStream = new MemoryStream();
			memoryStream.Write(bytes, 0, bytes.Length);
			memoryStream.Seek(0, SeekOrigin.Begin);
			_zipPackage.AddStream(memoryStream, String.Format("{0}.h", _grammarName));
		}
