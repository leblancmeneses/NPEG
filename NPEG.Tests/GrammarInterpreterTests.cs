...
namespace NPEG.Tests
{
    ...
    {
        public GrammarInterpreterTests()
        {
        }
        ...
        public void PEGrammar_Literal()
        {
            ...
            var caseSensitive = PEGrammar.Load(@"(?<Expression>): 'Hello World';");
            ...
            var notCaseSensitive = PEGrammar.Load(@"(?<Expression>): 'Hello World'\i;");
            ...
            input = @"\";
            var escape = PEGrammar.Load(@"(?<Literal>): ""\\"";");
            visitor = new NpegParserVisitor(new StringInputIterator(input));
        ...
        [TestMethod]
        [Timeout(2000)]
        [ExpectedException(typeof(ApplicationExceptions.InfiniteLoopDetectedException))]
        public void PEGrammar_ZeroOrMore_InfiniteLoopTest()
        {
            var input = "";
            var caseSensitive = PEGrammar.Load(@"(?<Expression>): (.*)*;");
        ...
        {
            var input = "";
            var caseSensitive = PEGrammar.Load(@"(?<Expression>): (.*){0,};");
...
#warning does not specify expected tree
        ...
        [TestMethod]
        [ExpectedException(typeof(NPEG.ApplicationExceptions.ParsingFatalTerminalException))]
