...
private:
  ...
  int _infiniteExpression() {
    return notPredicate((Npeg::IsMatchPredicate)&_ZeroTest::_expression);
  }

public:
  int isMatch() throw (ParsingFatalTerminalException) {
    return Npeg::zeroOrMore((Npeg::IsMatchPredicate)&_ZeroTest::_expression);
  }

  int bad() throw (ParsingFatalTerminalException) {
    return zeroOrMore((Npeg::IsMatchPredicate)&_ZeroTest::_infiniteExpression);
  }

public:
  _ZeroTest(InputIterator *iterator) : Npeg(iterator) {}
};
...
int main(int argc, char *argv[]) 
{
  ...
  p_context = new _ZeroTest(p_iterator);
