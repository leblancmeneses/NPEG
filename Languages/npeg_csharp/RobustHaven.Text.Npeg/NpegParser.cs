...
namespace RobustHaven.Text.Npeg
{
    ...
    public abstract class NpegParser
    {
        ...
        private IAstNodeFactory pointerAstNodeFactory = null;
        private Stack<Stack<AstNode>> sandbox = new Stack<Stack<AstNode>>();
        private Dictionary<String, Stack<Byte[]>> backReferenceLookup = new Dictionary<string, Stack<Byte[]>>();
        private Stack<Boolean> disableBackReferencePop = new Stack<bool>();


        public NpegParser(InputIterator iterator)
        {
            this.iterator = iterator;
            this.sandbox.Push(new Stack<AstNode>());
        }

        public NpegParser(InputIterator iterator, IAstNodeFactory createCustomAstNode)
        {
            this.iterator = iterator;
            this.pointerAstNodeFactory = createCustomAstNode;
            this.sandbox.Push(new Stack<AstNode>());
        }
        ...
        protected Boolean PrioritizedChoice(IsMatchPredicate left, IsMatchPredicate right)
        {
            ...
            this.sandbox.Push(new Stack<AstNode>());
            this.sandbox.Peek().Push(new AstNode());

            if (left())
            {
                Stack<AstNode> s = this.sandbox.Pop();
                if (s.Count >= 1)
                {
                    AstNode child = s.Pop();
                    if (this.sandbox.Peek().Count == 0)
                    {
                        this.sandbox.Peek().Push(child);
                    }
                    else
                    {
                        this.sandbox.Peek().Peek().Children.AddRange(child.Children);
                    }
                }
        ...
        {
            if (doCreateCustomNode == true && pointerAstNodeFactory == null)
            {
                throw new ArgumentNullException("Second constructor overload is required during instantiation.  Function pointer to AstNodeCreate method requires to be set with this parser implementation.");
            }
            ...
            if (expression())
            {
                ...
                {
                    this.backReferenceLookup[name].Push(matchedBytes);
                }
                else
                {
                    this.backReferenceLookup.Add(name, new Stack<Byte[]>());
                    this.backReferenceLookup[name].Push(matchedBytes);
                }


                AstNode node = this.sandbox.Peek().Pop();
                ...
                if (this.sandbox.Peek().Count > 0)
                {
                    node.Parent = this.sandbox.Peek().Peek();
                    this.sandbox.Peek().Peek().Children.Add(node);
                }
                else
                {
                    this.sandbox.Peek().Push(node);
        ...
        protected Boolean Literal(String matchText, Boolean isCaseSensitive)
        {
            foreach (Char c in matchText)
            {
                ...
                else
                {
                    ...
                    if (Encoding.ASCII.GetString(new Byte[] { (Byte)iterator.Current() }).ToString().ToUpper() != c.ToString().ToUpper())
                    {
                        iterator.Next();
                        return false;
                    }
        ...
        protected Boolean CodePoint(String matchCode, Int32 length)
        {
            if (matchCode.StartsWith("#x"))
            {
                ...
                for (int i = 0; i < total_compare_bytes; i++)
                {
                    ...
                    Byte compareWith = (Byte)iterator.Current();
                    ...
                    for (Int32 j = 0; j < 2; j++)
                    {
                        ...
                        {
                            if (
                                ((compareWith & (1 << (nibbleOffset + 3))) >> (nibbleOffset + 3)) != ((tmp & (1 << (nibbleOffset + 3))) >> (nibbleOffset + 3)) ||
                                ((compareWith & (1 << (nibbleOffset + 2))) >> (nibbleOffset + 2)) != ((tmp & (1 << (nibbleOffset + 2))) >> (nibbleOffset + 2)) ||
                                ((compareWith & (1 << (nibbleOffset + 1))) >> (nibbleOffset + 1)) != ((tmp & (1 << (nibbleOffset + 1))) >> (nibbleOffset + 1)) ||
            ...
            else if (matchCode.StartsWith("#b"))
            {
                ...
                if (binaryinput.Length % 8 != 0)
                    binaryinput = "".PadLeft(8 - (binaryinput.Length % 8), '0') + binaryinput; // on incomplete byte boundaries shift right
                ...
                for (int i = 0; i < total_compare_bytes; i++)
                {
                    ...
                    for (Int32 j = 0; j < 8; j++)
                    {
                        Byte tmp = (Byte)binaryinput[i * 8 + j];
                        if (tmp == (Byte)'0')
                        {
                            tmp = 0x00;
                        }
            ...
            else
            {
                ...
                if (decimalinput.Length > 10) // 4,294,967,295
                    throw new ArgumentException("Decimal codepoint value exceeds 4 byte maximum length.  Largest decimal value possible 2^32");

                UInt32 codepoint = UInt32.Parse(decimalinput);
                Byte c4 = (Byte)(codepoint >> 24);
                Byte c3 = (Byte)(codepoint >> 16);
                Byte c2 = (Byte)(codepoint >> 8);
                Byte c1 = (Byte)(codepoint);
                if (c4 != 0x00)
                {
                    // consumes 4 bytes
        ...
        {
            Byte[] matchBytes = new Byte[1];
            if (disableBackReferencePop.Count <= 0)
                matchBytes = this.backReferenceLookup[backReferenceName].Pop();
            else
                matchBytes = this.backReferenceLookup[backReferenceName].Peek();
        ...
        {
            this.Warnings.Add(new Warn(message, iterator.Index));
            return true;
        }
    }
}
