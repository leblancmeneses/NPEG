...
namespace NPEG
{
  public class NpegParserVisitor : IParseTreeVisitor
  {
    ...
    private readonly Stack<NpegLog> _npegLog = new Stack<NpegLog>();
    ...
    public NpegParserVisitor(IInputIterator iterator)
    {
      ...
      _npegLog.Push(new NpegLog());
    }

    public NpegParserVisitor(IInputIterator iterator, IAstNodeFactory astNodeFactory) : this(iterator)
    {
    ...
    public NpegLog NpegLog
    {
      get
      {
        if (_npegLog.Count == 1 && _npegLog.Peek().Children.Count > 0)
        {
          return _npegLog.Peek().Children[0];
        }
    ...
    public bool LogNonTerminal(AExpression command, IInputIterator inputIterator, Func<bool> isMatch)
    {
      if (IsOptimized)
      {
        return isMatch();
      ...
      log.Command = command;
      log.RuleStart = command.RuleStart;
      log.RuleEnd = command.RuleEnd;
      log.InputStart = inputIterator.Index;
      log.IsMatch = isMatch();
      log.InputEnd = inputIterator.Index;

      _npegLog.Pop();
      _npegLog.Peek().Children.Add(log);

      return log.IsMatch;
    }
    public bool LogTerminal(AExpression command, IInputIterator inputIterator, Func<bool> isMatch)
    {
      if (IsOptimized)
      {
        return isMatch();
...
#warning create another thread and process these simultaneously.
          ...
          delegate (IInputIterator iterator)
          {
            return LogNonTerminal(expression, iterator, () =>
                ...
                else
                {
                  ...
                  var varLengthSolver = new LimitingRepetitionVariableLengthExpressionSolver(varLengthIterator, iterator,
                            variableValues);
            ...
            return LogNonTerminal(expression, iterator, () =>
                ...
                if (expression.DoCreateCustomAstNode && _astNodeFactory == null)
                {
                  throw new ArgumentNullException(
                            "Second constructor overload is required during instantiation.  astNodeFactory requires to be set with this parser implementation.");
                }
    ...
    public override void VisitLeave(RecursionCreate expression)
    {
      ...
      if (!_recursiveMethod.ContainsKey(expression.FunctionName))
        _recursiveMethod.Add(
            ...
            delegate (IInputIterator iterator)
                {
                  return LogNonTerminal(expression, iterator, () => local(iterator));
                }
        );
    ...
    public override void Visit(Literal expression)
    {
      ...
      _matchStack.Push(
          delegate (IInputIterator iterator)
          {
            return LogTerminal(expression, iterator, () =>
                  {
    ...
    public override void Visit(RecursionCall expression)
    {
      ...
      _matchStack.Push(
          delegate (IInputIterator iterator)
          {
            return LogTerminal(expression, iterator, () => _recursiveMethod[delegatePointer](iterator));
          });
    }
      ...
      _matchStack.Push(
          delegate (IInputIterator iterator)
          {
            return LogTerminal(expression, iterator, () =>
                ...
                matchBytes = _xmlDisableBackReferencePop.Count <= 0 ? _xmlBackReferenceLookup[backreferencename].Pop() : _xmlBackReferenceLookup[backreferencename].Peek();
