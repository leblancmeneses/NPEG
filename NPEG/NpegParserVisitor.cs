...
namespace NPEG
{
	...
	{
		private readonly Stack<IsMatchPredicate> matchStack = new Stack<IsMatchPredicate>();
		...
		private readonly Dictionary<String, IsMatchPredicate> recursiveMethod =
			new Dictionary<string, IsMatchPredicate>();
		...
		private readonly Stack<Boolean> disableCapturingGroup = new Stack<bool>();
		...
		private readonly IAstNodeFactory astNodeFactory;

		public NpegParserVisitor(IInputIterator iterator)
		{
			this.iterator = iterator;
			Warnings = new List<Warn>();
			sandbox.Push(new Stack<AstNode>()); // initial base sandbox
			IsOptimized = false;
		}
		...
		public Boolean IsMatch
		{
			get
			{
				...
				{
					hasBeenMatched = true;
					matchedValue = matchStack.Peek()(iterator);
				}
		...
		{
			disableCapturingGroup.Push(true);
		}
		...
		{
			disableCapturingGroup.Pop();

			IsMatchPredicate exp = matchStack.Pop();
			matchStack.Push(
		...
		public override void VisitLeave(NotPredicate expression)
		{
			...
			IsMatchPredicate local = matchStack.Pop();
			matchStack.Push(
				delegate(IInputIterator iterator)
					{
						disableBackReferencePop.Push(true);
...
#warning create another thread and process these simultaneously.
		...
		{
			IsMatchPredicate localRight = matchStack.Pop();
			IsMatchPredicate localLeft = matchStack.Pop();
			matchStack.Push(
				delegate(IInputIterator iterator)
					{
						Int32 savePosition = iterator.Index;
		...
		{
			if (disableCapturingGroup.Count == 0)
				// no predicates being processed.
			{
				...
				matchStack.Push(
					delegate(IInputIterator iterator)
						{
							if (expression.DoCreateCustomAstNode && astNodeFactory == null)
							{
							...
							if (local(iterator))
							{
								...
								if (expression.DoCreateCustomAstNode)
								{
									...
									IAstNodeReplacement nodevisitor = astNodeFactory.Create(node);
		...
		{
			IsMatchPredicate local = matchStack.Pop();
			if (!recursiveMethod.ContainsKey(expression.FunctionName))
				recursiveMethod.Add(
			...
			matchStack.Push(recursiveMethod[expression.FunctionName]);
		}
		...
		public override void Visit(RecursionCall expression)
		{
			...
			matchStack.Push(
				delegate(IInputIterator iterator) { return recursiveMethod[delegatePointer](iterator); }
				);
		}
